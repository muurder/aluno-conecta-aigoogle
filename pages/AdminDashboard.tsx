...
const AdminDashboard: React.FC = () => {
    ...
    const handleReprove = (login: string) => {
        if (window.confirm(`Tem certeza que deseja reprovar (excluir) o usuário ${login}? Esta ação não pode ser desfeita.`)) {
            deleteUser(login);
            fetchUsers();
        }
    };
        ...
        <div className="min-h-full flex flex-col bg-gray-50">
            ...
            <main className="flex-grow p-4 space-y-3">
                {users.length === 0 ? (
                    ...
                    users.map(user => (
                        <div key={user.login} className="bg-white p-4 rounded-lg shadow-sm border space-y-3">
                            <div>
